...
| 상황 | 우선 검토할 선택 | 확인할 비용과 경계 |
|---|---|---|
| DB 제약조건 오류를 서비스 본문에서 확인해야 함 | 명시적 `flush` | 오류를 일찍 잡아도 실패한 트랜잭션은 rollback-only가 될 수 있음 |
| 벌크 연산 뒤 기존 관리 엔티티를 계속 사용해야 함 | 단일 대상은 `refresh`, 전체 재조회는 `clear` | refresh는 대상별 SQL, clear는 모든 관리 엔티티를 준영속화 |
| 한 행의 단순 조건과 변경 | 조건부 원자적 `UPDATE`와 영향 행 수 | 영속성 컨텍스트 우회, 복잡한 다중 상태 판단에는 별도 설계 필요 |
| 충돌은 드물지만 유실을 감지해야 함 | `@Version` 낙관적 락 | 충돌 시 최신 상태를 다시 읽고 전체 트랜잭션을 재시도할지 판단 |
| 충돌이 잦고 조회부터 계산까지 순서를 보장해야 함 | 비관적 쓰기 락 | 락 대기, 긴 보유 시간, 교착상태 위험 |
| 페이징 없이 부모와 컬렉션을 항상 함께 사용 | 컬렉션 Fetch Join | 한 번의 왕복 대신 자식 수만큼 늘어나는 조인 결과 행 |
| 부모 페이징과 컬렉션 로딩을 함께 수행 | 부모 페이징 + Batch Fetching | 배치 크기에 따른 추가 `IN` SQL 횟수 |
| 복잡한 부모 검색·정렬과 명시적인 객체 그래프 로딩 | 부모 ID 페이징 후 Fetch Join | 두 쿼리 관리, 정렬 복원, 빈 ID와 쿼리 사이 변경 가능성 |

모든 선택은 SQL 횟수만으로 결정하지 않고 DB 왕복 횟수, 반환 행 수, 생성되는 엔티티 수, 락 보유 시간과 실패 시 rollback 범위를 함께 비교한다.
